...
public partial class searchbyname : System.Web.UI.Page
{
    ...
    protected void LstOrder_ItemCommand(object sender, ListViewCommandEventArgs e)
    {
        ...
        if (e.CommandName == "Delete")
        {
            ...
            Cnn.ExecuteNonQuery("delete from [newsdetails] where newsid='" + LblOrderId.Text + "'");
            Cnn.ExecuteNonQuery("delete from [newsrelation] where blogid='" + LblOrderId.Text + "'");
    ...
    private void LoadData()
    {
        ...
        word = txtname.Text.Trim();
        
        if (txtname.Text != "")
        {
            mobile = " LEFT(name, 1) = '" + txtname.Text.Trim() + "'";
        }
       
        Cnn.Open();
        DataTable dtable = Cnn.FillTable("SELECT * FROM customerlist WHERE LEFT(name, 1) = '"+txtname.Text.Trim()+"'  ORDER BY id ASC;", "Order");
